...
- Total observations: `r nrow(bond_data)`
- Date range: `r min(bond_data$date, na.rm = TRUE)` to `r max(bond_data$date, na.rm = TRUE)`
- Symbols retrieved: `r length(unique(bond_data$symbol))`
- Observations with no data: `r bond_data |> filter(is.na(price)) |> nrow()`
...
After cleaning:
- Observations with no data: `r bond_data_clean |> filter(is.na(price)) |> nrow()`
