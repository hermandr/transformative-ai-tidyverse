...
Calculate in one parallel run using `furrr` package

Start time: `r  Sys.time()`

```{r}
#| label: setup-parallel-run
library(furrr)
...
Runing the calculations across `r availableCores() - 1` cores
...
placebo_data = placebo_replicates |> 
  ungroup() |> 
  select(-placebo_event_dates, -symbol) |> 
  unnest(plot_data)
```
