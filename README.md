...
Where: - $y_{t+s}$ = yield at day $s$ relative to event - $y_{t-b}$ = baseline yield $b$ days before event - $b$ = baseline period (5 or 15 days)
...
For questions about this replication: - Check the `AGENTS.md` files in each directory for context - Review the original paper for methodology details - Examine the scripts for implementation specifics
